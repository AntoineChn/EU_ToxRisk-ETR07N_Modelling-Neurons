...
# saveRDS(beta_logit_estimation, 'beta_logit_estimation.rds')
beta_logit_estimation = readRDS('beta_logit_estimation')

# Post analysis -----------------------------------------------------------

predictions <- rstan::extract(beta_logit_estimation,"Y_predict" ) %>% 
  melt()

predictions$Var2 %>% unique()
predictions$value
