---
title: "FBtalk_Fit_Analysis_Deguelin"
output:
  html_document:
    toc: true
    toc_depth: 3
...
#                                     chemi = tmp.chemi, 
#                                     extension = "stanFit")
tmp.stanFit.name = "step_by_step_uptoKE2_MTa_linear-ts_1909051420-chemi_Deguelin-chain_3-iter_10000-seed_1909051420-nEffMin_3311-rHatMax100_100.stanFit"
...
f.stanFit.filter(pattern = tmp.stanFit.ts[1],
                                    chemi = tmp.chemi,
                                    extension = "stanFit")
# tmp.stanFit.name = "step_by_step_uptoKE4_NRa_TwoKERs_PRa_Hierarchical_Exp_NRa_two_parents_mixExp-ts_1909061617-chemi_Deguelin-chain_3-iter_10000-seed_1909061617-nEffMin_2-rHatMax100_204.stanFit"
tmp.stanFit.name = "step_by_step_uptoKE4_NRa_TwoKERs_PRa_Hierarchical_Exp_NRa_two_parents_mixExp-ts_1909201236-chemi_Deguelin-chain_3-iter_10000-seed_1909201236-nEffMin_3-rHatMax100_185.stanFit"

# tmp.stanFit.name = "step_by_step_uptoKE4_NRa_TwoKERs_PRa_Hierarchical_Exp_NRa_two_parents_mixExp-ts_1909201525-chemi_Deguelin-chain_3-iter_10000-seed_1909201525-nEffMin_2-rHatMax100_1656.stanFit"
...
post_sample.PRa_NRa = tmp.fit %>% 
  ...
  bind_rows(tmp.est.ponct %>% 
              mutate(replication = "map")) %>% 
  mutate(y_min_CIa    = 	6.9487978      ,
         y_max_CIa    = 	99.0577596     ,
         log_x_50_CIa = 	log(0.0792933) ,
         k_CIa        = 	-1.7671816     ,
         beta_MTa     = 	0.820052       ,
         beta_0_MTa   = 	7.407383)
```
...
p.data_pred %>% 
  group_by(curve) %>% # group by post sample
  distinct(.,interaction(curve,Prot_acti), .keep_all = T) %>% # keep only one 0 value for Prot_acti
  distinct(.,interaction(curve,Neurite_Area), .keep_all = T) %>% # keep only one 0 value for Prot_acti
  top_n(-2, Neurite_Area) %>% # Keep the lowest two value of Neurite
  select(x_min_PRa_NRa, Prot_acti, Neurite_Area, curve)
```
...
p.data_pred %>% group_by(curve) %>% 
  summarise(avg = mean(mean_y_max_PRa))

p.data_pred %>% 
  filter(mito_resp == 100   , Neurite_Area < 99) %>%
  group_by(curve) %>% 
  top_n(1, curve) %>% 
  select(Neurite_Area,
...
box3d()
axes3d(edges=c('x--', 'x+-', 'y+-', 'z++',"z-+"))
# ?axes3d
text3d(x=  xlims[2]/2, y= -ylims[2]/9  , z= -zlims[2]/9  , "MTa"  , cex=1.4, col="black")
text3d(x= xlims[2]*6/5, y=  ylims[2]/2  , z= -zlims[2]/6  , "PRa"  , cex=1.4, col="black")
text3d(x= xlims[2]*6/5, y=  ylims[2]*6/5, z=  zlims[2]/2  , "NRa"  , cex=1.4, col="black")
