---
title: "morechemi_uptoKE1_CIa_loglogistic"
output:
  html_document:
    toc: true
    toc_depth: 3
...
```{r}
tmp.data.KE1 %>% select(chemi, chemiID) %>% 
  group_by(chemiID) %>% 
  count(chemi)
```
...
```{r}
tmp.data.KE1 %>% select(chemi, chemiID, concentration_MuMol) %>% 
...
```{r}
map(chemi.common.all$chemiID, function(x) {
  tmp.chemiID = x
  chemi.common.all %>%  filter(chemiID == tmp.chemiID)
