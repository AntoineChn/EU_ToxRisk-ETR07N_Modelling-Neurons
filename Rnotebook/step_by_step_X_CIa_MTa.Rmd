...
```{r eval = F}
fit_step_by_step_X_CIa_MTa
check_hmc_diagnostics(fit_step_by_step_X_CIa_MTa)
```
...
```{r}
tmp.MTa_pp <- rstan::extract(tmp.fit, "MTa_pred_MTa") %>%
  reshape2::melt()

tmp.concentration_MuMol = data.frame(
                            concentration_MuMol = tmp.data.KE2$concentration_MuMol,
                            Var2 = 1:length(tmp.data.KE2$concentration_MuMol))

tmp.MTa_pp = left_join(tmp.MTa_pp, tmp.concentration_MuMol, by = "Var2")
```



```{r}
tmp.est.ponct = la.map_all ; tmp.est.ponct %>% t()
...
tmp.MTa_pp %>%
  ...
  stat_function(fun = function(x) 
    f.link.lgstc(log(x),
                 params = tmp.pars_CIa) %>% 
      f.link.linear(., tmp.pars_MTa) %>% 
      f.link.truncation(.,lower = 0, upper = 120) ) +
  ggtitle("Actual MTa and posterior predictive replications wrt Dose") +
  annotate("text", x = 0.05, y = 80, label = "MAP prediction curve", hjust = 0) +
  annotate("text", x = 0.05, y = 60, label = "Posterior replications", colour = "orange", hjust = 0)
```
